...
- (NSMutableDictionary *)oldPlugIns { return [[oldPlugIns retain] autorelease]; }
...
- (void)loadInfo{
	...
	if (!plugInWebData) plugInWebData=[[NSMutableDictionary alloc]init];
}
...
- (NSMutableDictionary *)localPlugIns {
    return [[localPlugIns retain] autorelease]; 
}
...
- (NSMutableDictionary *)knownPlugIns {
    return [[knownPlugIns retain] autorelease]; 
}
...
- (NSString *)installPlugInFromFile:(NSString *)path{
	...
	if (![manager movePath:path toPath:destinationPath handler:nil])QSLog(@"move failed,%@,%@",path,destinationPath);
	[[NSWorkspace sharedWorkspace] noteFileSystemChanged:[path stringByDeletingLastPathComponent]];
...
- (NSMutableArray *)updatedPlugIns { return [[updatedPlugIns retain] autorelease]; }
...
- (void)setInstallProgress:(float)newInstallProgress {
	//QSLog(@"prof %f", newInstallProgress);
