...
void QSLogMesh(CGPointWarp **mesh,int width,int height){
	...
	for (i=0;i<width;i++){
		for (j=0;j<height;j++){
			NSLog(@"[%d][%d] - (%f,%f) (%f,%f)",i,j,0,0,0,0,mesh[i][j].local.x,mesh[i][j].local.y,mesh[i][j].global.x,mesh[i][j].global.y);	
...
CGPointWarp *QSTestMeshEffect(QSWindowAnimation *hp,float f,int *w, int*h){
	...
	*h=H;
	...
	//NSLog(@"xet %x %d",mesh,CGSSetWindowWarp(_CGSDefaultConnection(), [window windowNumber], W, H, mesh));
