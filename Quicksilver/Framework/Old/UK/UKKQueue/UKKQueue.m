...
-(void) dealloc
{
	...
	while( (fdNum = [enny nextObject]) )
	{
    	if( close( [fdNum intValue] ) == -1 )
            QSLog(@"dealloc: Couldn't close file descriptor (%d)", errno);
    }
...
-(void) removePathFromQueue: (NSString*)path
{
	...
	if( close( fd ) == -1 )
        QSLog(@"removePathFromQueue: Couldn't close file descriptor (%d)", errno);
}
...
-(void)		watcherThread: (id)sender
{
    ...
    while( keepThreadRunning )
    {
			...
			if( n > 0 )
			{
				if( ev.filter == EVFILT_VNODE )
				{
					if( ev.fflags )
					{
						...
						//QSLog(@"UKKQueue: Detected file change: %@", fpath);
		...
		NS_HANDLER
			QSLog(@"Error in UKKQueue watcherThread: %@",localException);
		NS_ENDHANDLER
	...
	if( close( theFD ) == -1 )
		QSLog(@"release: Couldn't close main kqueue (%d)", errno);
	
    //QSLog(@"exiting kqueue watcher thread.");
}
...
//      used by QSLog & co. when you request to print this object using the
//      %@ format specifier.
//
//	REVISIONS:
//		2004-11-12	UK	Created.
