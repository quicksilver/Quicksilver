...
- (NSString *)buildVersion{
	...
	NSString * version=[data objectForKey:(NSString *)kCFBundleVersionKey];
	return version;
...
- (NSString *)infoHTML{
	...
	if (!attributes)attributes=[[NSDictionary alloc]initWithObjectsAndKeys:
...
- (NSBundle *)bundle { return [[bundle retain] autorelease]; }
...
- (NSString *)loadError { return [[loadError retain] autorelease]; }
...
- (void)registerPlugInFrameworks{
	...
	if ([fm fileExistsAtPath:frameworksPath]){
		...
		//	QSLog(@"bund %@",[NSBundle bundleWithPath:testpath]);
		//[[NSBundle bundleWithPath:testpath]load];
		//		QSLog(@"%s",getenv("DYLD_FRAMEWORK_PATH"));
...
- (BOOL)_registerPlugIn{
	...
	foreachkey(key,handlerClass,[QSReg elementsForPointID:kQSPlugInInfoHandlers]){
