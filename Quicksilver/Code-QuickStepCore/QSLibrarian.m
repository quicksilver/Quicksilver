...
//	if (DEBUG_CATALOG)  NSLog(@"Saved Catalog in %f seconds",-[date timeIntervalSinceNow]);
//}
...
- (void)initCatalog{
	...
	//  if (!presets) NSLog(@"Unable to load presets");
...
- (float)estimatedTimeForSearchInSet:(id)set{
	...
	// if (VERBOSE)NSLog(@"Estimte: %fms avg: %d탎",estimate*1000,(int)(searchSpeed*1000000));
...
/*
 - (NSMutableArray *)scoredArrayForString:(NSString *)searchString inSet:(id)set mnemonicsOnly:(BOOL)mnemonicsOnly{
	 ...
	 //  if (VERBOSE)NSLog(@"Ranking: %fms avg: %d탎",-([date timeIntervalSinceNow]*1000),(int)(speed*1000000));date=[NSDate date];
	 [rankObjects sortUsingSelector:@selector(scoreCompare:)];
	 //  NSLog(@"rank %@",rankObjects);
	 return rankObjects;
 }
 */
