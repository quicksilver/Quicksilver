...
- (NSPasteboard *)getSelectionFromFrontApp {
	...
	[NSThread detachNewThreadSelector:@selector(invokeService) toTarget:self withObject:nil];
...
+(id)currentSelection {
	NSDictionary *info = [[QSReg tableNamed:@"QSProxies"] objectForKey:[[[NSWorkspace sharedWorkspace] activeApplication] objectForKey:@"NSApplicationBundleIdentifier"]];
	if (info) {
		return [[QSReg getClassInstance:[info objectForKey:kQSProxyProviderClass]] resolveProxyObject:nil];
	} else {
		NSPasteboard *pb = nil;
...
- (BOOL)bypassValidation {
	return ([[[[NSWorkspace sharedWorkspace] activeApplication] objectForKey:@"NSApplicationBundleIdentifier"] isEqualToString:@"com.blacktree.Quicksilver"]);
}
- (NSArray *)typesForProxyObject:(id)proxy {
	...
	else return [[QSReg getClassInstance:[info objectForKey:kQSProxyProviderClass]] typesForProxyObject:self];
}
@end
