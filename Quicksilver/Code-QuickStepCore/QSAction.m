...
- (id)initWithIdentifier:(NSString *)newIdentifier bundle:(NSBundle *)newBundle{
	...
	//				NSLog(@"rank %@ %d",self,rank);
	    ...
	    //NSLog([[NSBundle mainBundle]localizedStringForKey:newIdentifier value:newIdentifier table:@"ActionDescriptions"]);
    }
    return self;
}
...
- (void)setRankModification:(float)aRankModification {
	NSLog(@"setRankModification: deprecated");
...
- (NSBundle *)bundle { 
	...
	if (!bundle){
//		if (VERBOSE)NSLog(@"bundle not found %@");
...
//	NSLog(@"format %@ %@",[(QSAction *)self bundle],format);
