...
- (void)scheduleIdleTimer{
	...
	if (nextFire){
		...
		//if (VERBOSE) 
		//NSLog(@"Next Idle in %f (%f)",idleRemaining,QSCurrentIdleTime());
...
-(void)checkIdle{
	...
	//		if (VERBOSE)NSLog(@"Idle for %f minutes",(float)idle/MINUTES );
	...
	//if (VERBOSE)
	//	NSLog(@"idles matching predicate %@\r%@",predicate,shouldHaveFired);
...
- (void)cancelPreviousPerformRequestsWithTarget:(id)aTarget selector:(SEL)aSelector object:(id)anArgument{
	foreach(callback,callbacks){
		...
		if ([aTarget isEqual:[callback objectForKey:@"target"]]
			...
			NSLog(@"removing callback %@",callback);
			[callbacks removeObject:callback];
		}
	}
}
