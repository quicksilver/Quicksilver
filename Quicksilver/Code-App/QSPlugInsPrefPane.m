...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
	...
	//	NSLog(@"select %@",[selection lastObject]);
...
- (float)outlineView:(NSOutlineView *)outlineView heightOfRowByItem:(id)item{
	...
	//NSLog(@"item %@",[item observedObject]);
	if ([[[item respondsToSelector:@selector(representedObject)]?[item representedObject]:[item observedObject] objectForKey:@"type"]isEqualToString:@"category"])
		return 16;	
	return 32;
}
...
//				break;
	...
	//if (VERBOSE) NSLog(@"Plugins Predicate: %@",filterPredicate);
