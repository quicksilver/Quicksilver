...
- (IBAction)donateOther:(id)sender{
	NSString *urlString=[NSString stringWithFormat:@"http://qs0.blacktree.com/quicksilver/donate.php?method=Mail&for=Quicksilver&amount=%d",[self donationAmount]];
	[[NSWorkspace sharedWorkspace]openURL:[NSURL URLWithString:urlString]];
}

- (IBAction)donatePayPal:(id)sender{
	
	NSString *urlString=[NSString stringWithFormat:@"http://qs0.blacktree.com/quicksilver/donate.php?method=PayPal&for=Quicksilver&amount=%d",[self donationAmount]];
	[[NSWorkspace sharedWorkspace]openURL:[NSURL URLWithString:urlString]];
}
- (id)init {
