...
+ (void)initialize{
	...
	//   NSLog(@"-------score: %f",[@"Let's go to the library" scoreForAbbreviation:@"letliby"]);
...
-(id) init{
	if (self=[super init]){
		...
		if ([[NSDate date]timeIntervalSinceDate:expireDate]>0){
			[NSApp activateIgnoringOtherApps:YES];
			NSLog(@"Quicksilver Expired %@",expireDate);
		...
		if (DEBUG_STARTUP) NSLog(@"Images loaded");
...
- (void)recompositeIconImages{
	...
	//NSLog(@"images %@ %@",[NSImage imageNamed:@"Quicksilver-Activated"],[NSImage imageNamed:@"Quicksilver-Running"]);
	...
	//	NSLog(@"hue %f sat %f",hue,saturation);
	//saturation =0.5;
	hue-=20.0/360;
	...
	//NSLog(@"nmages %@ %@",newActivated,newRunning);
...
- (void)setShowMenuIcon:(NSNumber *)mode{
	...
	[statusItem retain];
	//	NSLog(@"priority %d",[statusItem priority]);
...
- (IBAction)donate:(id)sender{
	NSString *baseURL=@"http://qs0.blacktree.com/quicksilver/contribute.php";
...
- (IBAction)getMorePlugIns:(id)sender{
	...
	//	NSString *baseURL=@"http://qs0.blacktree.com/quicksilver/plugins.php?feature=";
...
- (void)displayStatusMenuAtPoint:(NSPoint)point{
	NSLog(@"display %f.%f",point.x,point.y);
...
//		//NSLog(@"opaque %d",[content clearsBackground]);
...
- (void)connectionDidInitialize:(NSNotification*)notif{
	//if (VERBOSE) NSLog(@"Connection Initialized: %x",[notif object]);
}
- (void)connectionDidDie:(NSNotification*)notif{
	//if (VERBOSE) NSLog(@"Connection Died: %x",[notif object]);
}
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)sender{
	...
	//  NSLog(@"%@",[NSImage imageNamed:@"NSApplicationIcon"])
...
- (void)startQuicksilver:(id)sender{
	...
	//NSLog(@"port %d",[[serviceConnection receivePort]machPort]);
   ...
   withKeyPath:@"values.QSActivationHotKey"
	...
	if (visiblePref){
	//	NSLog(@"reopeningpref %s",visiblePref);
