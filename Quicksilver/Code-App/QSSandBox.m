...
void listEncodings(){
	...
	for (int i=0; encodings[i] != kCFStringEncodingInvalidId;i++) {
		...
		if (i && encodings[i]-encodings[i-1]>16)NSLog(@"-");
		NSLog(@"Enc: %x %@",encodings[i],encodingName);
    }
}
...
//	
//	NSLog(@"X scanning with %@",parser);
//	
//	usleep(5);
//	//for(int i=0;i<5;i++){
//		NSLog(@"start scanning with %@",parser);
...
//			NSLog(@"done %d items",[objects count]);//,[objects count]);
//				
//			[pool release];
//		}
//		//	[pool release];
...
//	}
	...
	//NSLog(@"screen %@",[string componentsSeparatedByStrings:[NSArray arrayWithObjects:@">>\r", @"\r", @" ", nil]]);
...
//	[[NSClassFromString(@"QSPowerManager") sharedInstance]listEvents];
	...
	//	NSLog(@"image %@",[[NSWorkspace sharedWorkspace] iconForFile:@"/Volumes/Lore/Desktop/test1"]);
	...
	//	if (DEBUG)NSLog(@"Dict %@",[[NSApp parentProcessInformation]);
	//	listEncodings();
	...
	//NSLog(@"",result);
	return;	
	//NSLog(@"code %d",[[[[QSKeyCodeTranslator alloc]init]autorelease]keyCodeForCharacter:@"c"]);
	//NSLog(@"code %d",[[[[QSKeyCodeTranslator alloc]init]autorelease]AsciiToKeyCode:118]);
