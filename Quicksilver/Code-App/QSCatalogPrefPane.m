...
//	NSLog(@"hu %f %f %f %f",hue, saturation,brightness, alpha);
   ...
   [catalogSetsTable setHighlightColor:highlightColor];
    ...
    NSMutableArray *sources=[[[[QSReg objectSources]allKeys]mutableCopy]autorelease];
...
- (void) selectEntry:(QSCatalogEntry *)entry{
	//if (VERBOSE)
	//NSLog(@"entry %@ %@ %@",self,[entry catalogSetIndexPath],[entry catalogIndexPath]);
	id section=nil;
...
objectValueForTableColumn:(NSTableColumn *)aTableColumn
            row:(int)rowIndex
{
	//NSLog(@"tree %@",[treeController valueForKeyPath:@"selection.contents"]);
...
- (BOOL) selectedCatalogEntryIsEditable{
	...
	//  NSLog(@"isPreset? %d %d",DEBUG,[librarian entryIsPreset:currentItem]);
...
- (BOOL)outlineView:(NSOutlineView *)aTableView itemIsSeparator:(id)item{
	...
	//NSLog(@"%@",[[treeController arrangedObjects]objectAtIndexPath:[item indexPath]]);
	return NO;
...
- (BOOL)outlineView:(NSOutlineView *)outlineView acceptDrop:(id <NSDraggingInfo>)info item:(id)item childIndex:(int)index{
	...
	if ([info draggingSourceOperationMask]==NSDragOperationMove
		...
		//	NSLog(@"dragged %@",draggedIndexPaths);
...
- (IBAction)setValueForSenderForCatalogEntry:(id)sender{
	if (sender==itemNameField){
		...
		NSData *imageData=[[sender image]TIFFRepresentationUsingCompression:NSTIFFCompressionLZW factor:nil];
...
- (void)catalogIndexed:(NSNotification *)notification{
	//NSLog(@"notobj:%@",[notification object]);
...
//	// NSLog(@"adjust with size %f",oldSize.width);
//	
//	[sender adjustSubviews];
//}
//
//- (void)splitViewDidResizeSubviews:(NSNotification *)aNotification{
//	//     NSLog(@"split %f",NSWidth([[[catalogSplitView subviews]lastObject]frame])/NSWidth([catalogSplitView frame]));
...
- (void)setCurrentItemContents:(NSArray *)newCurrentItemContents {
	//	NSLog(@"netcont %@",newCurrentItemContents);
