...
- (QSObject *) performAction:(QSAction *)action directObject:(QSBasicObject *)dObject indirectObject:(QSBasicObject *)iObject{
	...
	if (![workspace unmountAndEjectDeviceAtPath:firstFile]){
		...
		//NSLog([NSString stringWithFormat:@"tell application \"Finder\" to eject disk \"%@\"",displayName]);
...
- (NSArray *)universalApps { 
	if (!universalApps){
		...
		//NSLog(@"Universal Apps: %@\r%d",[universalApps valueForKeyPath:@"path.lastPathComponent"],[universalApps count]);
	...
	if ([action isEqualToString:kFileOpenWithAction]){
		...
		/*
		 ...
		 for (i=0;i<[appURLs count];i++){
			 ...
			 if (valid){NSLog(@"url%@",[appURLs objectAtIndex:i]);   
...
- (NSArray *)validActionsForDirectObject:(QSObject *)dObject indirectObject:(QSObject *)iObject{
	...
	//NSLog(@"%@ %@",path,[[NSFileManager defaultManager] typeOfFile:path]);
