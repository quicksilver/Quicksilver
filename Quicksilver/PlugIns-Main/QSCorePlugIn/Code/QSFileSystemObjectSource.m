...
- (void)populateFields{
    NSFileManager *manager=[NSFileManager defaultManager];
	//   NSLog(@"Populate With %@",[self currentEntry]);
...
- (IBAction)setValueForSender:(id)sender{
    ...
    if (sender==typeSetsPopUp){
		...
		*/  
        ...
        //NSLog(@"%@",[[sender string]componentsSeparatedByString:@","]);
...
- (NSString *)fullPathForSettings:(NSDictionary *)settings{
    ...
    if (![itemPath isAbsolutePath]){
		//NSLog(@"base %@",[settings objectForKey:kItemBaseBundle]);
...
- (BOOL)indexIsValidFromDate:(NSDate *)indexDate forEntry:(NSDictionary *)theEntry{
    ...
    NSNumber *depth=[settings objectForKey:kItemFolderDepth];
		...
		//NSLog(@"depth of %@ %@",itemPath, indexDate);
		//NSDate *date=[NSDate date];
		  NSDate *modDate=[manager path:itemPath wasModifiedAfter:indexDate depth:[depth intValue]];
		//  NSLog(@"nepth %@ - %f",modDate,[date timeIntervalSinceNow]);
		  //date=[NSDate date];
		  //modDate=[manager bulkPath:itemPath wasModifiedAfter:indexDate depth:[depth intValue]];
		 // NSLog(@"d`epth %@ - %f",modDate,[date timeIntervalSinceNow]);
		  return modDate==nil;
		  // //  NSLog(@"depth %@ %d",modDate,[modDate compare:indexDate]!=NSOrderedDescending);
