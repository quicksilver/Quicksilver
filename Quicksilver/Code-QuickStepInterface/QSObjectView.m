...
- (NSDragOperation)draggingUpdated:(id <NSDraggingInfo>)sender{
	...
	if (operation==NSDragOperationGeneric){
	   ...
	   // NSLog(@"dragg %@ %d %d",[sender draggingSource], sourceDragMask,operation);	
	return operation;
}
...
//	NSLog(@"drag %d %d",sourceDragMask,NSDragOperationGeneric);
	[self invalidateSpringTimer];
	if (shouldSpring){
...
- (void)concludeSpringWithData:(id)object{
	//	NSLog(@"concludeSpringed %@",[NSApp nextEventMatchingMask:NSLeftMouseUpMask untilDate:[NSDate dateWithTimeIntervalSinceNow:2.5] inMode:NSDefaultRunLoopMode dequeue:YES]);
	...
	[self dragImage:[[self draggedObject] icon] at:[self convertPoint:[[self window]mouseLocationOutsideOfEventStream]fromView:nil] offset:NSZeroSize
			  event:[NSEvent mouseEventWithType:NSLeftMouseDown location:NSZeroPoint modifierFlags:nil timestamp:nil  windowNumber:[[self window]windowNumber] context:nil eventNumber:nil clickCount:0 pressure:0]
