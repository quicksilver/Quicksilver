...
+ (NSImage *)imageWithCIFilter:(CIFilter *)filter{
	return [self imageWithCIImage:[filter valueForKey:@"outputImage"]];
}
...
- (NSImage *)imageByAdjustingHue:(float)hue{
	...
	CIFilter *hueAdjust = [CIFilter filterWithName:@"CIHueAdjust"
									 keysAndValues:	
		@"inputAngle",[NSNumber numberWithFloat:hue*2*M_PI],
		@"inputImage",[CIImage imageWithData:[self TIFFRepresentation]],
		nil];
	return [NSImage imageWithCIFilter:hueAdjust];	
}
...
- (NSImage *)imageByAdjustingHue:(float)hue saturation:(float)saturation{
	...
	CIFilter *satAdjust = [CIFilter filterWithName:@"CIColorControls"
		...
		@"inputBrightness",[NSNumber numberWithFloat:0.0f],
		@"inputContrast",[NSNumber numberWithFloat:1.0f],
		@"inputImage",[hueAdjust valueForKey:@"outputImage"],
		nil];
...
- (BOOL)createRepresentationOfSize:(NSSize)newSize{ 
    ...
    NSBitmapImageRep* iconRep = [[[NSBitmapImageRep alloc]initWithFocusedViewRect:NSMakeRect(0,0,newSize.width,newSize.height)]autorelease];
...
-(NSRect)usedRect{
    ...
    [bitmap release];
    //flip y!!
	//NSLog(@"%d,%d,%d,%d",minX,minY,maxX,maxY);
